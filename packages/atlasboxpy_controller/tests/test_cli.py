...
from atlasboxpy_controller.cli import main

# Resolve the console script relative to the running interpreter, not via
# shell PATH lookup — `subprocess.run(["atlasboxpy-controller", ...])` only
...
_CLI_SCRIPT = str(Path(sys.executable).parent / "atlasboxpy-controller")
...
def test_help_lists_init_subcommand():
    ...
    # specifically about `atlasboxpy-controller --help`, not just main()).
...
def test_generated_controller_imports_and_runs(tmp_path, monkeypatch):
...
def test_add_feature_generated_controller_imports_and_runs(tmp_path, monkeypatch):
