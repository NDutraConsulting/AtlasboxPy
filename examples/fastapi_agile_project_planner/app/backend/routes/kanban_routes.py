...
# Field names never written to the traffic log in cleartext — matched
# case-insensitively against each top-level key in `props`. This log is
# a persistent, unencrypted file (see logging_setup.py); FindRelatedTasksRequest.token
# (a session-token/JWT stand-in — see UserSessionService's docstring) is
# the first field in this app that's genuinely credential-shaped, but the
# check is generic so any future field named like a credential is covered
# by construction, not by remembering to special-case each route.
_SENSITIVE_KEYS = {"token", "password", "secret", "authorization", "api_key"}
_REDACTED = "***REDACTED***"


def _redact_sensitive(props: dict[str, Any]) -> dict[str, Any]:
    return {
...
async def _call(request: Request, method: _ControllerMethod) -> JSONResponse:
    ...
    _traffic_log.info(
        ...
        method.__name__,
        status,
        json.dumps(_redact_sensitive(props)),
...
def build_routes(controller: KanbanController) -> list[Route]:
    ...
    async def find_related_tasks_by_card(request: Request) -> JSONResponse:
        return await _call(request, controller.find_related_tasks_by_card)
    ...
    return [
        ...
        Route(
            "/api/cards/tag-by-team", find_related_tasks_by_card, methods=["POST"]
        ),
