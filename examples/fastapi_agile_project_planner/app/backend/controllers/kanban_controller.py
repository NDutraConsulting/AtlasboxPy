...
class FindRelatedTasksRequest(BaseModel):
    """find_related_tasks_by_card — a session token standing in for a
    real Authorization header/JWT (see UserSessionService's own
    docstring for why it's a stub)."""
...
class KanbanController(BaseController):
    def __init__(self) -> None:
        ...
        self.user_sessions = UserSessionService()
    ...
    #
    # find_related_tasks_by_card is this app's worked example of what
    # atlasboxpy_service's README calls a controller's job that isn't a
    # service's: UserSessionService, KanbanService, and TaskAgentService
    # each own exactly one bounded concern and never call each other —
    # only this method calls all three, in sequence, because each step
    ...
    #
    # No step here is independent of the others, so there's no
    # gather_named opportunity in this particular example — see
    # KanbanService's own get_board/list_boards for where concurrent
    # orchestration actually applies.

    async def find_related_tasks_by_card(
        self, props: dict[str, Any]
    ) -> SuccessResponse[Any] | ErrorResponse:
        payload = validate_props(FindRelatedTasksRequest, props)

        async with _tracer.span("find_related_tasks_by_card"):
            user = await self.user_sessions.get_user(payload.token)
            if user is None:
                return build_error_response(
                    NotFoundError("Unknown or expired session token")
                )

            cards_response = self._response_for(
                await self.service.get_cards_by_team(user.team_id)
            )
            if isinstance(cards_response, ErrorResponse):
                return cards_response

            tag_map = await self.task_agent.tag_cards(cards_response.data)
            return self._response_for(await self.service.update_card_tags(tag_map))
