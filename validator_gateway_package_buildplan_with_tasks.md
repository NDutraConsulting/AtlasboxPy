...
- [x] **P9-T1: `examples/fastapi_basic/`**
...
- [x] **P9-T2: `examples/worker_recovery/`**
...
- [x] **P11-T6: `validator_gateway.classifying.ClassifyingValidatorGateway` + `validator-gateway add-feature <name>`**
  **Do:** A pattern proven out in `examples/fastapi_kanban` (per-feature `*ValidatorGateway` subclasses that override `handle()` with a visible `FailureCase` enum + `match/case`, redirect to another gateway, and a required `source_json` the caller declares) turned out valuable enough to promote: `ClassifyingValidatorGateway` — with `SourceJson` — is a proper package export (`validator_gateway.classifying`), same tier as `recovery`/`fastapi_integration` (opt-in, not in the top-level namespace). `_severity_fallback()` and `_resolve()` are `abstractmethod`s, so a subclass literally cannot be instantiated without implementing both — the classification logic can never be silently skipped, and it stays visible and required in the *developer's own file*, not buried, even though the plumbing (try/except, severity fallback, an always-log guarantee via `logging.getLogger("validator_gateway.traffic")`) now lives in the package. `add-feature <name> [--path PATH] [--force]` scaffolds `controllers/{name}_controller.py` + `validator_gateways/{name}_validator_gateway.py` for one feature (importing `ClassifyingValidatorGateway`/`SourceJson` straight from the package — nothing else is generated).
  **File(s):** `src/validator_gateway/classifying.py`, `src/validator_gateway/cli.py`, `tests/test_classifying.py`, `tests/test_cli.py`
  **Acceptance:** A subclass missing `_severity_fallback`/`_resolve` raises `TypeError` on instantiation. `add-feature invoice` produces a gateway that imports cleanly and correctly classifies a `NotFoundError` as its `NOT_FOUND` case; running it twice for two different feature names never collides; re-running for the same name without `--force` refuses and leaves hand-edits untouched; a non-snake_case name is rejected before anything is written. Core test suite (including `test_classifying.py`) still passes with `fastapi` never installed.
