...
def configure_telemetry_logging() -> None:
    """Same handler, same rotation, a separate file — atlasboxpy_telemetry's
    own README notes that pointing a log shipper at its logger's output is
    a deployment concern the package itself doesn't handle; this is this
    demo's own choice of "where," same as configure_traffic_logging()
    above. Spans only actually get logged when telemetry is enabled (see
    atlasboxpy_telemetry.is_enabled()) — this just decides where they land
    when they are."""
    logger = logging.getLogger("atlasboxpy_telemetry")
    logger.setLevel(logging.INFO)
    logger.propagate = False
    if not any(isinstance(h, _DailyRotatingFileHandler) for h in logger.handlers):
        logger.addHandler(_DailyRotatingFileHandler(_LOG_DIR, "atlasboxpy_telemetry.log"))
