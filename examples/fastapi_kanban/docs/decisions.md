...
ADRs specific to this example app — not `atlasboxpy_controller`'s or
`atlasboxpy_repository`'s own choices (see those packages' own
`docs/decisions.md`), but what this demo does with them.
...
└─ Need data from more than one entity (a board's columns and cards, or
   ...
   repository, or to fetch the pieces one `await` after another?
     └─→ ADR-3: no to both — a repository (`BoardRepository`/
          ...
          concurrently (`asyncio.gather`) since the lookups don't
