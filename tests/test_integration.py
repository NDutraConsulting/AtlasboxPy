"""P8-T2: a full layered lifecycle — route -> gateway -> controller ->
service -> repository (in-memory fake) -> back through error/success
formatting — for a success path and one path per major DomainError
subclass. Deliberately separate from examples/, which are runnable demos
...
def build_app() -> FastAPI:
    ...
    app = FastAPI()
    router = APIRouter()
    gateway_dep = get_gateway_factory(lambda: UserController(service))

    @router.post("/users/{user_id}")
    async def create_user(
        user_id: str, name: str, gateway: ValidatorGateway = Depends(gateway_dep)
    ):
        result = await gateway.handle(gateway.controller.create_user, user_id, name)
        return to_json_response(result)

    @router.post("/users/{user_id}/banned")
    async def create_banned_user(
        user_id: str, name: str, gateway: ValidatorGateway = Depends(gateway_dep)
    ):
        result = await gateway.handle(gateway.controller.create_banned_user, user_id, name)
        return to_json_response(result)
    ...
    async def get_user_unauthenticated(
        user_id: str, gateway: ValidatorGateway = Depends(gateway_dep)
    ):
        result = await gateway.handle(gateway.controller.get_user_unauthenticated, user_id)
        return to_json_response(result)

    @router.post("/users/{user_id}/publish")
    async def publish_user(
        user_id: str, version: int, gateway: ValidatorGateway = Depends(gateway_dep)
    ):
        result = await gateway.handle(gateway.controller.publish_user, user_id, version)
        return to_json_response(result)

    @router.post("/users/{user_id}/sync")
    async def sync_user(user_id: str, gateway: ValidatorGateway = Depends(gateway_dep)):
